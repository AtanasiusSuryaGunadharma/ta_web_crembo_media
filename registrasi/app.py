...
def get_pengumuman_items(days_after=3):
    ensure_table_kegiatan()

    with sqlite3.connect(DB_NAME) as conn:
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute("""
            SELECT id, judul, tanggal, misa_json
            FROM kegiatan
    ...
    items = []
    for r in rows:
        if not kegiatan_masih_aktif(r["tanggal"], r["misa_json"], days_after=days_after):
            continue

        try:
            misa_list = json.loads(r["misa_json"] or "[]")
        except Exception:
            misa_list = []

        # urutkan misa by misa_ke lalu jam (stabil)
        def misa_sort_key(m, idx):
            mk = m.get("misa_ke") or 0
            jm = (m.get("jam") or "").strip()
            mk_int = int(mk) if str(mk).isdigit() else 0
            return (mk_int, jm, idx)

        misa_list = sorted(
            [(m, i) for i, m in enumerate(misa_list)],
            key=lambda t: misa_sort_key(t[0], t[1])
        )
        misa_list = [m for m, _ in misa_list]

        items.append({
            "id": r["id"],
            "judul": r["judul"],
            "tanggal": r["tanggal"],
            "tanggal_sort": _parse_tanggal(r["tanggal"]),
            "misa_list": misa_list
        })

    items.sort(key=lambda x: (x["tanggal_sort"], x["id"]))
    for it in items:
        it.pop("tanggal_sort", None)
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        c.execute("""
            CREATE TABLE IF NOT EXISTS kegiatan (
                ...
                misa_json TEXT NOT NULL DEFAULT '[]',
...
def soft_delete_kegiatan(kegiatan_id: int):
    ensure_table_kegiatan()
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        c.execute("""
            ...
            WHERE id = ?
        """, (STATUS_DELETED, now, kegiatan_id))
        conn.commit()
    return True


def get_anggota_options():
    with sqlite3.connect(DB_NAME) as conn:
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute("SELECT username, nama FROM anggota ORDER BY username ASC")
        rows = c.fetchall()
    return [{"username": r["username"], "nama": r["nama"]} for r in rows]

def parse_misa_from_post(form):
    misa_ke_list = form.getlist('misa_ke[]')
    jam_list     = form.getlist('jam[]')

    dp_list      = form.getlist('dp_username[]')
    op_list      = form.getlist('op_username[]')
    vmix_list    = form.getlist('vmix_username[]')

    n = len(misa_ke_list)
    misa_rows = []

    for i in range(n):
        kamera = [x.strip() for x in form.getlist(f'kamera[{i}][]') if x and x.strip()]
        sup    = [x.strip() for x in form.getlist(f'supervisor[{i}][]') if x and x.strip()]
        foto   = [x.strip() for x in form.getlist(f'fotografer[{i}][]') if x and x.strip()]

        misa_ke_raw = (misa_ke_list[i] if i < len(misa_ke_list) else "").strip()
        jam_raw     = (jam_list[i] if i < len(jam_list) else "").strip()

        if not jam_raw:
            raise ValueError("Jam misa wajib diisi untuk setiap misa.")

        misa_rows.append({
            "misa_ke": int(misa_ke_raw) if misa_ke_raw.isdigit() else (i + 1),
            "jam": jam_raw,
            "dp_username": (dp_list[i] if i < len(dp_list) else "").strip(),
            "op_username": (op_list[i] if i < len(op_list) else "").strip(),
            "vmix_username": (vmix_list[i] if i < len(vmix_list) else "").strip(),
            "kamera": kamera,
            "supervisor": sup,
            "fotografer": foto
        })
...
def dashboard():
    ...
    items_pengumuman = get_pengumuman_items(days_after=3)

    return render_template(
        'dashboard.html',
        role=session.get('role'),
        nama=nama,
        items_pengumuman=items_pengumuman
    )
...
#        c = conn.cursor()
#        c.execute("""
#            SELECT id, judul, tanggal, status, updated_at
#            FROM kegiatan
#            ORDER BY tanggal DESC, id DESC
#        """)
#        rows = c.fetchall()
#
#    return render_template('admin_kegiatan_list.html', rows=rows)
...
def admin_kegiatan_list():
    ...
    # pastikan konstanta ada (kalau belum kamu define di atas)
    try:
        _ = STATUS_DELETED
    except NameError:
        # fallback supaya route tidak error
        # (tetap lebih bagus kalau kamu define global STATUS_DELETED = "deleted")
        STATUS_DELETED = "deleted"
...
# ============================================================================ROUTE LEGACY
@app.route('/admin/kegiatan/legacy')
def admin_kegiatan_legacy():
    if 'username' not in session or session.get('role') != 'admin':
        return redirect(url_for('login'))

    init_kegiatan_table()  # tabel kegiatan_form lama
    ...
    return render_template('admin_kegiatan_legacy_list.html', data=data)


# ---------------------------------------------------------
# ALIAS ENDPOINT: biar url_for('admin_kegiatan') tidak error
# Tanpa mengubah route yang sudah ada.
# ---------------------------------------------------------
@app.route('/admin/kegiatan_alias')
def admin_kegiatan():
    return redirect(url_for('admin_kegiatan_list'))
...
def admin_kegiatan_form():
    ...
    anggota_options = get_anggota_options()

    # Optional: mode edit via ?kegiatan_id=ID
    kegiatan_id = request.args.get('kegiatan_id', '').strip()
    kegiatan = None
    misa_list = []
    ...
    if kegiatan_id:
        with sqlite3.connect(DB_NAME) as conn:
            conn.row_factory = sqlite3.Row
            c = conn.cursor()
            c.execute("SELECT * FROM kegiatan WHERE id = ?", (kegiatan_id,))
            row = c.fetchone()

        if not row:
            flash("Data kegiatan tidak ditemukan.", "error")
            return redirect(url_for('admin_kegiatan'))

        kegiatan = {"id": row["id"], "judul": row["judul"], "tanggal": row["tanggal"], "status": row["status"]}
        try:
            misa_list = json.loads(row["misa_json"] or "[]")
        except Exception:
            misa_list = []
    ...
    if request.method == 'POST':
        ...
        action = request.form.get('action', 'draft').strip()  # draft / publish
        status = 'publish' if action == 'publish' else 'draft'

        if not judul or not tanggal:
            flash("Judul dan tanggal wajib diisi.", "error")
            return render_template(
        ...
        try:
            misa_rows = parse_misa_from_post(request.form)
        except ValueError as e:
            flash(str(e), "error")
            # tampilkan ulang form dengan input terakhir
            kegiatan_fallback = kegiatan or {"judul": judul, "tanggal": tanggal}
            return render_template(
                'admin_kegiatan_form.html',
                kegiatan=kegiatan_fallback,
                misa_list=misa_rows if 'misa_rows' in locals() else misa_list,
                anggota_options=anggota_options
            )
        ...
        if kegiatan_id:
            with sqlite3.connect(DB_NAME) as conn:
                ...
                c.execute("""
                    UPDATE kegiatan
                    SET judul = ?, tanggal = ?, status = ?, misa_json = ?, updated_at = ?
                    WHERE id = ?
                """, (judul, tanggal, status, json.dumps(misa_rows, ensure_ascii=False), now, kegiatan_id))
                conn.commit()
            flash("Draft diperbarui." if status == "draft" else "Kegiatan dipublish.", "success")
        else:
            with sqlite3.connect(DB_NAME) as conn:
                c = conn.cursor()
                c.execute("""
                    INSERT INTO kegiatan (judul, tanggal, status, misa_json, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (judul, tanggal, status, json.dumps(misa_rows, ensure_ascii=False), now, now))
                conn.commit()
            flash("Draft tersimpan." if status == "draft" else "Kegiatan dipublish.", "success")
...
#  DELETE KEGIATAN
#=========================================================
@app.route('/admin/kegiatan/delete/<int:kegiatan_id>', methods=['POST', 'GET'])
def admin_kegiatan_delete(kegiatan_id):
    if 'username' not in session:
        return redirect(url_for('index'))
    if session.get('role') != 'admin':
        abort(403)

    soft_delete_kegiatan(kegiatan_id)
    flash("Kegiatan berhasil dihapus dari daftar admin.", "success")
    return redirect(url_for('admin_kegiatan_list'))
...
@app.route('/admin/kegiatan/v2')
def admin_kegiatan_v2_list():
    if 'username' not in session or session.get('role') != 'admin':
        return redirect(url_for('login'))
    ...
    return render_template('admin_kegiatan_v2_list.html', rows=rows)

#==============  EDITING ================================
@app.route('/admin/kegiatan/edit/<int:kegiatan_id>', methods=['GET', 'POST'])
def admin_kegiatan_edit(kegiatan_id):
    ...
    if request.method == 'POST':
        ...
        action  = request.form['action']  # draft / publish
        status  = 'publish' if action == 'publish' else 'draft'

        # update kegiatan utama
        cur.execute("""
            UPDATE kegiatan
            SET judul=?, tanggal=?, status=?
            WHERE id=?
        """, (judul, tanggal, status, kegiatan_id))

        # hapus semua misa lama (AMAN & DISENGAJA)
        cur.execute("DELETE FROM kegiatan_misa WHERE kegiatan_id=?", (kegiatan_id,))

        # insert ulang dari form
        misa_ke_list = request.form.getlist('misa_ke[]')
        jam_list     = request.form.getlist('jam[]')
        dp_list      = request.form.getlist('dp_username[]')
        op_list      = request.form.getlist('op_username[]')
        vmix_list    = request.form.getlist('vmix_username[]')

        for i in range(len(misa_ke_list)):
            cur.execute("""
                INSERT INTO kegiatan_misa
                (kegiatan_id, misa_ke, jam, dp_username, op_username, vmix_username)
                VALUES (?,?,?,?,?,?)
            """, (
                kegiatan_id,
                misa_ke_list[i],
            ...
            # multi roles
            for u in request.form.getlist(f'kamera[{i}][]'):
                if u:
                    cur.execute("INSERT INTO kegiatan_petugas_multi VALUES (?,?,?)",
                                (misa_id, 'kamera', u))
            for u in request.form.getlist(f'supervisor[{i}][]'):
                if u:
                    cur.execute("INSERT INTO kegiatan_petugas_multi VALUES (?,?,?)",
                                (misa_id, 'supervisor', u))
            for u in request.form.getlist(f'fotografer[{i}][]'):
                if u:
                    cur.execute("INSERT INTO kegiatan_petugas_multi VALUES (?,?,?)",
                                (misa_id, 'fotografer', u))

        conn.commit()
        flash('Kegiatan berhasil diperbarui')
        return redirect(url_for('admin_kegiatan_list'))

    # ================= GET (prefill form) =================

    cur.execute("SELECT * FROM kegiatan WHERE id=?", (kegiatan_id,))
    kegiatan = cur.fetchone()

    cur.execute("""
        SELECT km.*, 
               GROUP_CONCAT(CASE WHEN kpm.role='kamera' THEN kpm.username END) AS kamera,
               GROUP_CONCAT(CASE WHEN kpm.role='supervisor' THEN kpm.username END) AS supervisor,
               GROUP_CONCAT(CASE WHEN kpm.role='fotografer' THEN kpm.username END) AS fotografer
        FROM kegiatan_misa km
        LEFT JOIN kegiatan_petugas_multi kpm ON kpm.kegiatan_misa_id = km.id
    ...
    for r in cur.fetchall():
        misa_list.append({
            'misa_ke': r['misa_ke'],
            'jam': r['jam'],
            'dp_username': r['dp_username'],
            'op_username': r['op_username'],
            'vmix_username': r['vmix_username'],
            'kamera': (r['kamera'] or '').split(',') if r['kamera'] else [],
            'supervisor': (r['supervisor'] or '').split(',') if r['supervisor'] else [],
            'fotografer': (r['fotografer'] or '').split(',') if r['fotografer'] else [],
        })

    anggota_options = get_all_anggota()
...
def get_misa_terakhir_dt(tanggal_str: str, misa_json_str: str):
    try:
        base_date = datetime.strptime(tanggal_str, "%Y-%m-%d").date()
    except Exception:
        return None

    try:
        misa_list = json.loads(misa_json_str or "[]")
    except Exception:
        misa_list = []

    max_time = None
    for m in misa_list:
        jam = (m.get("jam") or "").strip()
        try:
            t = datetime.strptime(jam, "%H:%M").time()
    ...
    if max_time is None:
        max_time = datetime.strptime("00:00", "%H:%M").time()

    return datetime.combine(base_date, max_time)

def kegiatan_masih_aktif(tanggal_str: str, misa_json_str: str, days_after: int = 3) -> bool:
    dt = get_misa_terakhir_dt(tanggal_str, misa_json_str)
    if not dt:
        return False
    return datetime.now() <= (dt + timedelta(days=days_after))

@app.route('/api/pengumuman/status')
def api_pengumuman_status():
    ensure_table_kegiatan()
    ...
    active = []
    for r in rows:
        if kegiatan_masih_aktif(r["tanggal"], r["misa_json"], days_after=3):
            active.append({"id": r["id"], "judul": r["judul"], "tanggal": r["tanggal"]})
...
#    items = []
#    for r in rows:
#        if not kegiatan_masih_aktif(r["tanggal"], r["misa_json"], days_after=3):
#            continue
#
#        try:
#            misa_list = json.loads(r["misa_json"] or "[]")
...
#            misa_list = []
#
        # urutkan misa by misa_ke lalu jam (biar enak dibaca)
#        def misa_sort_key(m):
#            mk = m.get("misa_ke") or 0
#            jm = m.get("jam") or ""
#            return (int(mk) if str(mk).isdigit() else 0, jm)
#
#        misa_list = sorted(misa_list, key=misa_sort_key)
#
#        items.append({
#            "id": r["id"],
#            "judul": r["judul"],
...
#
#    if not items:
#        return "Tidak ada pengumuman aktif.", 404
#
#    return render_template("pengumuman_public.html", items=items)
from datetime import datetime, date
import json
import sqlite3
...
@app.route('/pengumuman')
def pengumuman_public():
    items = get_pengumuman_items(days_after=3)
    if not items:
        return "Tidak ada pengumuman aktif.", 404
    return render_template("pengumuman_public.html", items=items)
