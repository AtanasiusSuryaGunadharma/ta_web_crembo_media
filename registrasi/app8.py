...
def update_profil():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        c = conn.cursor()
        c.execute("""
            SELECT nama, tgl_lahir, telp, email, password
            FROM anggota
            WHERE username = ?
        """, (username,))
        row = c.fetchone()
    ...
    if request.method == 'POST':
        ...
        pass_lama = request.form.get('pass_lama', '').strip()
        pass_baru = request.form.get('pass_baru', '').strip()
        pass_konfirmasi = request.form.get('pass_konfirmasi', '').strip()

        # DEBUG opsional untuk cek autofill (hapus atau biarkan komentar)
        # print("DEBUG pass_lama:", repr(pass_lama))
        # print("DEBUG pass_baru:", repr(pass_baru))
        # print("DEBUG pass_konfirmasi:", repr(pass_konfirmasi))

        # user dianggap mau ganti password kalau mengisi salah satu field password
        mau_ganti_password = bool(pass_lama or pass_baru or pass_konfirmasi)

        try:
            with sqlite3.connect(DB_NAME) as conn:
                c = conn.cursor()

                # Update profil (tanpa password) selalu boleh
                c.execute("""
                    UPDATE anggota
                    SET nama = ?, tgl_lahir = ?, telp = ?, email = ?
                    WHERE username = ?
                """, (nama_baru, tgl_lahir_baru, telp_baru, email_baru, username))

                # Kalau user MAU ganti password, baru verifikasi password lama
#                conn.commit()

        except Exception as e:
            # jaga-jaga kalau ada error sqlite lainnya
            print("ERROR update profil:", e)
            flash("Terjadi kesalahan saat menyimpan profil.")
            return redirect(url_for('update_profil'))

        flash("Profil berhasil diperbarui.")
        return redirect(url_for('profil'))

    # GET: tampilkan form isi data lama
    return render_template(
        'update_profil.html',
        nama=nama_lama or '',
        tgl_lahir=tgl_lahir_lama or '',
        telp=telp_lama or '',
        email=email_lama or ''
    )
