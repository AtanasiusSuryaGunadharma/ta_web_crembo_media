...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        # Penting: aktifkan foreign keys
        conn.execute("PRAGMA foreign_keys = ON;")
        c = conn.cursor()

        # --- TABEL UTAMA ANGGOTA (sesuai file asli) ---
        ...
        # --- STREAMING_OPREC: jadwal pelatihan, FK via telepon ---
        c.execute("""
        CREATE TABLE IF NOT EXISTS streaming_oprec (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            telepon   TEXT NOT NULL,          -- FK ke oprec(telepon)
            posisi    TEXT NOT NULL,          -- Operator/Kameramen/Supervisor
            nama      TEXT,                   -- diisi otomatis dari oprec
            created_at TEXT DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (telepon) REFERENCES oprec(telepon)
              ON UPDATE CASCADE
              ON DELETE RESTRICT
        );
        """)

        # --- TRIGGER: auto-isi nama di streaming_oprec dari oprec ---
        c.execute("""
        CREATE TRIGGER IF NOT EXISTS trg_streaming_oprec_autonama
        AFTER INSERT ON streaming_oprec
        FOR EACH ROW
        BEGIN
          UPDATE streaming_oprec
          SET nama = (SELECT nama_lengkap FROM oprec WHERE telepon = NEW.telepon)
          WHERE id = NEW.id;
        END;
        """)

        # --- INDEX bantu ---
        c.execute("CREATE INDEX IF NOT EXISTS idx_oprec_telepon ON oprec(telepon);")
        c.execute("CREATE INDEX IF NOT EXISTS idx_streaming_oprec_telepon ON streaming_oprec(telepon);")
...
def login():
    if request.method == 'POST':
        ...
        if user:
            stored_hash = user[4]  # kolom ke-5 = password_hash (mengikuti file asli)
